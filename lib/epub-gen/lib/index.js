    ...
    downloadAllImage() {
      ...
      if (!self.options.images.length) {
        ...
        var task = function () {
          ...
          var job = [];
          var thisCount = 10;
          for (; thisCount && count < self.options.images.length; count++) {
            const image = self.options.images[count];
            console.log('[Downloading]', image.url);
            job.push(self.downloadImage(image));
            thisCount--;
          }
          return job.length ? Q.all(job).then(task) : false;
        };

        Q.fcall(task).catch(function(error) {
          console.log(error);
        }).fin(function() {
